...
function LibFroznFunctions:RefreshAnchorShoppingTooltips(tip)
	...
	-- Protect against secret values from GetLeft/GetRight/GetEffectiveScale
	local leftPos, rightPos
	pcall(function()
		leftPos = (sideAnchorFrame:GetLeft() ~= nil) and (sideAnchorFrame:GetLeft() * sideAnchorFrame:GetEffectiveScale())
		rightPos = (sideAnchorFrame:GetRight() ~= nil) and (sideAnchorFrame:GetRight() * sideAnchorFrame:GetEffectiveScale())
	end)

	-- local selfLeftPos = tooltip:GetLeft(); -- removed
	-- local selfRightPos = tooltip:GetRight(); -- removed
	-- Protect against secret values from GetLeft/GetRight/GetEffectiveScale
	local selfLeftPos, selfRightPos
	pcall(function()
		selfLeftPos = (tooltip:GetLeft() ~= nil) and (tooltip:GetLeft() * tooltip:GetEffectiveScale())
		selfRightPos = (tooltip:GetRight() ~= nil) and (tooltip:GetRight() * tooltip:GetEffectiveScale())
	end)
...
function LibFroznFunctions:RecalculateSizeOfGameTooltip(tip)
	...
	-- Protect against secret values from GetPadding
	pcall(function()
	...
	if (not eventsForGroupRosterUpdateRegistered) then
		...
		local function cachePlayerGuildClubMemberInfosFn()
			...
			if (playerGuildClubIDCache) then
				-- Protect against secret values from GetClubMembers - wrap entire operation in pcall
				pcall(function()
					local playerGuildClubMemberIDs = C_Club.GetClubMembers(playerGuildClubIDCache);

					if playerGuildClubMemberIDs and type(playerGuildClubMemberIDs) == "table" then
...
function LibFroznFunctions:GetTalents(unitID)
	-- check if talents are available
	local success, isSelf = pcall(UnitIsUnit, unitID, "player");
	isSelf = success and isSelf;
