...
local function AddTarget(lineList,target,targetName)
	local success, isPlayer = pcall(UnitIsUnit, "player", target);
...
function ttStyle:GeneratePlayerLines(tip, currentDisplayParams, unitRecord, first)
	...
	if (cfg.showStatus) then
		-- Protect against secret boolean values from Unit status functions
		local status
		pcall(function()
			status = (not UnitIsConnected(unitRecord.id) and " <DC>") or (UnitIsAFK(unitRecord.id) and " <AFK>") or (UnitIsDND(unitRecord.id) and " <DND>")
		end)
		if (status) then
